...
use crate::misc::{errortypes::validation_error::ValidationError, datetime_utils::{DAYS_MAP, DatetimeUtils, WORKINGHOURS_AFTER, WORKINGHOURS_BEFORE}};
...
impl WorkhoursValidator {
    ...
    fn created_on_workday(datetime: SystemTime) -> Result<(), ValidationError> {
        ...
        if let Some((_, _)) = dur {
            ...
            return Err(ValidationError::InvalidWorkingDaysDate { found: dt.format("%Y-%m-%d %H:%M:%S").to_string() })
        }
    }
    ...
    fn created_during_workhours(datetime: SystemTime) -> Result<(), ValidationError>
    {
        ...
        if DatetimeUtils::has_surpassed_hours(datetime, before) && !DatetimeUtils::has_surpassed_hours(datetime, after) {
            Ok(())
        }
        else {
            Err(ValidationError::InvalidWorkingDaysDate { found: dt.format("%Y-%m-%d %H:%M:%S").to_string() })
        }
    }
}