...
mod syntax_validator_tests {
    ...
    fn test_syntax_localdatetime_validator(given_datetime: &str, expected_datetime: SystemTime, success: bool) {
        match success {
            true => assert_eq!(expected_datetime, DatetimeValidator::validate(given_datetime).unwrap()),
            false => assert_ne!(expected_datetime, DatetimeValidator::validate(given_datetime).unwrap())
        };
    }
}